...
class _TrackingPageState extends State<TrackingPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title:  MainHeader(
            title: "Visit Tracking".tr(), subTitle: "Monitor your daily visits".tr()),
      ),
      body: Selector<TrackingProvider, bool>(
          builder: (context, provide, widget) {
            return provide
                ? const TrackingShimmer()
                : Padding(
                          ...
                          children: [
                            Selector<TrackingProvider, int>(
                              ...
                              builder: (context, selectedIndex, _) {
                                return Row(
                                  children: [
                                    _tabItem(context, "Pending".tr(), 0,
                                        selectedIndex, trackingProvider),
                                    const SizedBox(width: 12),
                                    _tabItem(context, "Completed".tr(), 1,
                                        selectedIndex, trackingProvider),
                                  ],
                                );
                              },
  ...
  Widget _tabItem(BuildContext context, String title, int index,
      int selectedIndex, TrackingProvider provider) {
